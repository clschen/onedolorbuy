...
<style type="text/css">
	*{margin:0;padding:0;}
	...
	.box{ border:5px solid #22AAFF; width:400px;height: 300px; background-color:#fff;
		  margin-top:15%;overflow:hidden;		
	}
	.box-b{width:400px;overflow:hidden;height: 300px;}
	/*.box-title{ background:<?php if(isset($config['titlebg'])): ?><?php echo $config['titlebg']; ?><?php  else: ?>#22AAFF<?php endif; ?>;height:30px;line-height:25px;_line-height:25px;font-size: 14px;color:<?php if(isset($config['title'])): ?><?php echo $config['title']; ?><?php  else: ?>#fff<?php endif; ?>; padding:0 10px;font-weight:600;}*/
	.box-title{ background:#22AAFF;height:30px;line-height:25px;_line-height:25px;font-size: 14px;color:<?php if(isset($config['title'])): ?><?php echo $config['title']; ?><?php  else: ?>#fff<?php endif; ?>; padding:0 10px;font-weight:600;}
	.box-text{font:12px/1.5 "微软雅黑",Arial,"宋体",Helvetica,sans-serif; font-size:18px;color:#22AAFF;width:400px;		
