...
class autolottery extends SystemAction {
	...
	public function autolottery_ret_install()
	{
		...
		if(!$shop_info){
			response::show(2004,'没有查到该条信息');
		}			

		if($shop_info['xsjx_time'] > time()){
			response::show(2004,'还未到揭晓时间');
		}	

		if($shop_info['canyurenshu']=='0'){
			response::show(2004,'还未有人参与');
		}			

		if(!empty($shop_info['q_user_code']) && ($shop_info['q_showtime'] == 'Y')){				
			response::show(2004,'已经揭晓过');
		}			

		if(!empty($shop_info['q_user_code']) && ($shop_info['q_showtime'] == 'N')){				
			response::show(2000,'获取信息成功',array('huode'=>$shop_info['q_user_code']));
		}
		...
		if($u_go_info){			
			...
			$q_1 = $this->db->Query("UPDATE `@#_shoplist` SET `q_uid` = '$q_uid',`q_user` = '$q_user',`q_user_code` = '$code',`q_content`	= '$content',`q_counttime` ='$counttime',`q_end_time` = '$shop_info[xsjx_time]' where `id` = '$id'");
			...
			if($q_1 && $q_2 && $q_3){				
				$this->db->Autocommit_commit();
				response::show(2000,'揭晓成功');
			}else{
				$this->db->Autocommit_rollback();
				response::show(2004,'揭晓失败');			
			}					
	}
