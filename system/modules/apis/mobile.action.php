...
class mobile extends base {
	...
	{
		$_POST = ['page'=>1,'pagesize'=>3,'types'=>10];
		if(empty($_POST['page']) || empty($_POST['pagesize']) || empty($_POST['types'])){
			response::show(2001,'缺少参数');
		}
		
		$select =htmlspecialchars($_POST['types']);
		$p =htmlspecialchars($_POST['page']) ? $_POST['page'] :1;
	...
	public function fen()
	{
		if(empty($_POST['fenid'])){
			response::show(2001,'缺少参数');
		}
		$fenid =htmlspecialchars($_POST['fenid']);
		...
		$list=count($shoplist);
		$data = compact($key,$shoplist,$list);
		if($data){
		   	response::show(2000,'获取信息成功',$data);
		}else{
		   	response::show(2004,'获取信息失败');
	...
	public function gonggaoxq()
	{
		...
		$ggid =htmlspecialchars($_POST['ggid']);
		$gonggaoxq=$this->db->GetList("SELECT * FROM `@#_article` WHERE `id` = $ggid");
		if($gonggaoxq){
		   	response::show(2000,'获取信息成功',$gonggaoxq);
		}else{
		   	response::show(2004,'获取信息失败');
		}
	...
	{
		$gonggaoxq=$this->db->GetList("SELECT * FROM `@#_article` WHERE `cateid` = 142");
		
		foreach($gonggaoxq as $k=>$v){
			$data[$k]['id'] = $v['id'];
	...
	public function goodsdesc()
	{
		...
		$itemid=intval($_POST['goods_id']);
		$desc=$this->db->GetOne("select content from `@#_shoplist` where `id`='$itemid'");
		if($desc){
		   	response::show(2000,'获取信息成功',$desc);
		}else{
		   	response::show(2004,'获取信息失败');
		}
	}
	...
	public function goodspost(){
		...
		// $shaidan['pinglun_sum'] = $sum;
		if(!empty($shaidan)){
		   	response::show(2000,'获取信息成功',$shaidan);
		}else{
		   	response::show(2004,'获取信息失败');
		}
	...
	public function about()
	{
		...
		if($article){
		   	response::show(2000,'获取信息成功',$article);
		}else{
		   	response::show(2004,'获取信息失败');
		}
	}
	...
	public function nextautolottery()
	{
		...
		$mingri_shoplist = $this->db->GetList("select * from `@#_shoplist` where `xsjx_time` > '$w_minri_time' and `xsjx_time` < '$w_houri_time' order by xsjx_time limit 0,3 ");
		if($mingri_shoplist){
		   	response::show(2000,'获取信息成功',$mingri_shoplist);
		}else{
		   	response::show(2004,'获取信息失败');
		}
	}
